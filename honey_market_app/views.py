...
def home_view(request):
    return render(request, 'honey_market_app/home.html')


class ProductListView(ListView): # afiseaza lista de obiecte
    model = Product
    template_name = 'honey_market_app/products.html'

    def get_context_data(self, **kwargs): # date suplimentare catre sablon, pagina products.html
        context = super().get_context_data(**kwargs)
        context['products'] = context['object_list']
        return context


def product_detail(request, product_id): # informatii despre produs dupa id
    product = get_object_or_404(Product, pk=product_id)
    return render(request, 'honey_market_app/product.html', {'product': product})


def upload_product(request): # incarcare produse in aplicatia noastra folosind un sablon, pagina upload_product.html
    ...
    return render(request, 'honey_market_app/upload_product.html', {'form': form})

@login_required
def add_to_cart(request, product_id): # adaugare produse in cosul de cumparaturi
    try:
        # produsul pe baza id-ului
        product = get_object_or_404(Product, id=product_id)

        # Verificare existanta comanda produs de la utilizator
        ...
        if created:
            ...
            # Actualizare cantitate la un produs existent in comanda
            order_item.quantity = F('quantity') + 1
            order_item.save()
            messages.info(request, f"Cantitatea pentru {product.name} a fost actualizată în coș.")

        # Redirectionare catre cosul de cumparaturi, pagina cart
        return redirect(reverse('honey_market_app:cart'))
...
def create_order(request):
    ...
    return render(request, 'honey_market_app/cart.html', context)
...
def finalizeaza_comanda(request):
    if request.method == 'POST':
        ...
        return redirect(reverse('honey_market_app:home'))
...
def contact(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            mesaj_nou.save()
            return redirect('/')  # Redirectionare la pagina de home
    else:
        form = MesajForm()

    return render(request, 'honey_market_app/contact.html', {'form': form})
